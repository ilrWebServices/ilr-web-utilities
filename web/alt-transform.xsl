...
<xsl:stylesheet version="1.0" xmlns:dm="http://www.digitalmeasures.com/schema/data" xmlns:dmd="http://www.digitalmeasures.com/schema/data-metadata" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="dm:Data">
    <faculty><xsl:text>
      ...
      <xsl:for-each select="//dm:Record">
        ...
        <xsl:choose>
          <xsl:when test="@username='mew15' or @username='emb6' or @username='mhd11' or @username='oji2' or @username='cm226' or @username='lrm32' or @username='plr27' or @username='rrs3' or @username='has34' or not(dm:INTELLCONT[dm:CONTYPE=''][dm:PUBLIC_VIEW='Yes'] != '')"/>
          ...
          <xsl:otherwise>
            <person>
              ...
              <name>
                <xsl:apply-templates select="dm:PCI/dm:FNAME"/><xsl:text> </xsl:text>
                <xsl:apply-templates select="dm:PCI/dm:LNAME"/>
              </name><xsl:text>
              </xsl:text>
              ...
              <publications>
                <xsl:if test=". != ''">
                  ...
                  <!-- Look for books and other ublications whose content type is not set. -->
                  <xsl:if test="dm:INTELLCONT[dm:CONTYPE=''][dm:PUBLIC_VIEW='Yes'] != ''">
                    <h4>Unspecified Publications</h4><xsl:text>
                    </xsl:text>
                    <ul class="pubs"><xsl:text>
                    </xsl:text>
                      <xsl:apply-templates select="dm:INTELLCONT[dm:CONTYPE=''][dm:PUBLIC_VIEW='Yes']">
  ...
  <xsl:template match="dm:PCI/dm:FNAME">
    <xsl:apply-templates/>
  </xsl:template>

  <xsl:template match="dm:PCI/dm:LNAME">
    <xsl:apply-templates/>
  </xsl:template>
  ...
  <xsl:template match="dm:INTELLCONT[dm:CONTYPE = ''][dm:PUBLIC_VIEW='Yes']">
    <xsl:if test=". != ''">
      <xsl:call-template name="outputpub">
        <xsl:with-param name="pubtype"></xsl:with-param>
      </xsl:call-template>
    </xsl:if>
  </xsl:template>
